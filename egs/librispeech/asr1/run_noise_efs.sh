...
train_config=conf/train3.yaml # current default recipe requires 4 gpus.
                             # if you do not have 4 gpus, please reconfigure the `batch-bins` and `accum-grad` parameters in config.
lm_config=conf/lm.yaml
decode_config=conf/decode_exp.yaml
...
# storage
efs="/efs/exp/clai24/feats/atis-SNSD/v1"
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for x in atis.valid_noise atis.test3_noise atis.train_noise; do
    ...
    dump.sh --cmd "$train_cmd" --nj ${nj} --do_delta ${do_delta} \
        data/${train_set}/feats.scp data/${train_set}/cmvn.ark exp/dump_feats/train_noise ${feat_tr_dir}
    dump.sh --cmd "$train_cmd" --nj ${nj} --do_delta ${do_delta} \
        data/${train_dev}/feats.scp data/${train_set}/cmvn.ark exp/dump_feats/dev_noise ${feat_dt_dir}
    for rtask in ${recog_set}; do
        feat_recog_dir=${dumpdir}/${rtask}/delta${do_delta}; mkdir -p ${feat_recog_dir}
        dump.sh --cmd "$train_cmd" --nj ${nj} --do_delta ${do_delta} \
            data/${rtask}/feats.scp data/${train_set}/cmvn.ark exp/dump_feats/recog_noise/${rtask} \
            ${feat_recog_dir}
    done
fi
...
if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
    echo "stage 7: decode BERT and Scoring"
    if [[ $(get_yaml.py ${train_config} model-module) = *transformer*  ]]; then
        if ${use_valbest_average}; then
            recog_model=model.val${n_average}.avg.best
            opt="--log ${expdir}/results/log"
    ...
    for rtask in ${recog_set}; do
        ...
        python ${slots_home}/clean_up_hypothesis.py ${expdir}/${decode_dir}/results.wrd.md ${slots_home}/${expdir}/${decode_dir}/cleaned-results.wrd.md
        slots_run=/home/ubuntu/tools/slot_filling_and_intent_detection_of_SLU
        ${slots_run}/run/atis_with_pure_bert2.sh --hypothesis_file ${slots_home}/${expdir}/${decode_dir}/cleaned-results.wrd.md \
                --slots_prediction_file ${slots_home}/${expdir}/${decode_dir}/hypothesis_slots_prediction.txt
