...
def get_parser(parser=None, required=True):
    ...
    parser.add_argument("--dict2", help="Dictionary")
    ...
    parser.add_argument(
        ...
        type=float,
        help="Multitask learning coefficient, "
        "alpha2: alpha2*ctc_loss2 + (1-alpha2)*att_loss2 ",
    )
    parser.add_argument(
        "--jointbeta1",
        default=1.0,
        type=float,
        help="joint optimization weighting objective"
    )
    parser.add_argument(
        "--jointbeta2",
        default=1.0,
    ...
    parser.add_argument(
        "--focal-alpha", default=1.0, type=float, help="focal loss alpha"
    )
    parser.add_argument(
        "--focal-gamma", default=2.0, type=float, help="focal loss gamma"
    )
    ...
    ####################### Below is for IC/SL #############################
    parser.add_argument('--SL-pretrained-model-type', required=True, help='bert, xlnet')
...
def main(cmd_args):
    ...
    # load dictionary for debug log
    if args.dict2 is not None:
        with open(args.dict2, "rb") as f:
            dictionary = f.readlines()
        char_list2 = [entry.decode("utf-8").split(" ")[0] for entry in dictionary]
        char_list2.insert(0, "<blank>")
        char_list2.append("<eos>")
        args.char_list2 = char_list2
    else:
        args.char_list2 = None
    print(args.char_list2)
    print(len(args.char_list2)) # 141
    # train
