...
class LoadInputsAndTargets(object):
    ...
    def __call__(self, batch):
        ...
        text_dict = OrderedDict()  # OrderedDict[str, List[np.ndarray]]
        uttid_list = []  # List[str]
        ...
        if self.mode == "asr":
            return_batch, uttid_list = self._create_batch_asr(
                x_feats_dict, y_feats_dict, text_dict, uttid_list
            )
        elif self.mode == "tts":
            _, info = batch[0]
    ...
    def _create_batch_asr(self, x_feats_dict, y_feats_dict, text_dict, uttid_list):
        ...
        if self.sort_in_input_length:
            ...
            #print('non zero sorted index is', nonzero_sorted_idx)
        else:
        ...
        if self.load_output:
            ...
            return_batch = OrderedDict(
                [*[(x_name, x) for x_name, x in zip(x_names, xs)], (y_name, ys), (text_name, texts)]
            )
        else:
