...
class LoadInputsAndTargets(object):
    ...
    def __call__(self, batch):
        ...
        y2_feats_dict = OrderedDict()  # OrderedDict[str, List[np.ndarray]]
        y2_feats_raw_dict = OrderedDict()  # OrderedDict[str, List[np.ndarray]]
        y3_feats_dict = OrderedDict()  # OrderedDict[str, List[np.ndarray]]
        y3_feats_raw_dict = OrderedDict()  # OrderedDict[str, List[np.ndarray]]
        text_dict = OrderedDict()
        token_dict = OrderedDict()
        select_token_id_dict = OrderedDict()
        uttid_list = []  # List[str]
        ...
        for uttid, info in batch:
            ...
            if self.load_output:
                ...
                for idx, inp in enumerate(info["output2"]):
                    #print(idx, inp)
                    #print(inp["slots"].split())
                    #print([self.tag_to_idx[x] for x in inp["slots"].split()])
                    if "slots" in inp:
                        # ======= Legacy format for output =======
                        # {"output": [{"tokenid": "1 2 3 4"}])
                        x = np.fromiter(
                            map(int, [self.tag_to_idx[x] for x in inp["slots"].split()]), dtype=np.int64
                        )
                    y2_feats_dict.setdefault(inp["name"], []).append(x)
                    if "slots" in inp:
                        # ======= Legacy format for output =======
                        # {"output": [{"tokenid": "1 2 3 4"}])
                        x = inp['slots']
                    y2_feats_raw_dict.setdefault(inp["name"] + '_raw', []).append(x)

                for idx, inp in enumerate(info["output2"]):
                    if "class" in inp:
                        ref_class = inp['class']
                        if '#' in ref_class:
                            class_label =(int(self.class_to_idx[ref_class.split('#')[0]]), ref_class.split('#'))
                        else:
                            class_label = int(self.class_to_idx[ref_class])
                        x = class_label
                        y = ref_class

                    y3_feats_dict.setdefault(inp["name2"], []).append(x)
                    y3_feats_raw_dict.setdefault(inp["name2"] + '_raw', []).append(y)
                for idx, inp in enumerate(info["output"]):
                    ...
                    if "tokenid" in inp:
                        ...
                        txt = [word.lower() for word in inp["text"].split()]
                        tokens = [token for token in inp["token"].split()]
                        select_token_id = inp["token_select_id"]
                    else:
                        # ======= New format =======
                        # {"input":
                        #  [{"feat": "some/path.h5:F01_050C0101_PED_REAL",
                    ...
                    text_dict.setdefault('text', []).append(txt)
                    token_dict.setdefault('token', []).append(tokens)
                    select_token_id_dict.setdefault('token_select_id', []).append(select_token_id)

        if self.mode == "asr":
            #print('into asr mode')
            return_batch, uttid_list = self._create_batch_asr(
                x_feats_dict, y_feats_dict, y2_feats_dict, y2_feats_raw_dict, y3_feats_dict, y3_feats_raw_dict,
                text_dict, token_dict, select_token_id_dict, uttid_list
            )
            #print(return_batch)
            #print(uttid_list)
        elif self.mode == "tts":
    ...
    def _create_batch_asr(self, x_feats_dict, y_feats_dict, y2_feats_dict, y2_feats_raw_dict, y3_feats_dict, y3_feats_raw_dict,
                          text_dict, token_dict, select_token_id_dict, uttid_list):
        ...
        if self.load_output:
            if len(y_feats_dict) == 1 and len(y2_feats_dict) == 1:
                ys = list(y_feats_dict.values())[0]
                ys2 = list(y2_feats_dict.values())[0]
                ys2_raw = list(y2_feats_raw_dict.values())[0]
                ys3 = list(y3_feats_dict.values())[0]
                ys3_raw = list(y3_feats_raw_dict.values())[0]
                assert len(xs[0]) == len(ys), (len(xs[0]), len(ys))

                texts = list(text_dict.values())[0]
                assert len(xs[0]) == len(texts), (len(xs[0]), len(texts))

                tokens = list(token_dict.values())[0]
                assert len(xs[0]) == len(tokens), (len(xs[0]), len(tokens))

                select_token_id = list(select_token_id_dict.values())[0]
                assert len(xs[0]) == len(select_token_id), (len(xs[0]), len(select_token_id))

                # get index of non-zero length samples
                nonzero_idx = list(filter(lambda i: len(ys[i]) > 0, range(len(ys))))
                nonzero_idx2 = list(filter(lambda i: len(ys2[i]) > 0, range(len(ys2))))
                assert nonzero_idx == nonzero_idx2
        ...
        if self.load_output:
            if len(y_feats_dict) == 1:
                ...
                ys2 = [ys2[i] for i in nonzero_sorted_idx]
                ys2_raw = [ys2_raw[i] for i in nonzero_sorted_idx]
                ys3 = [ys3[i] for i in nonzero_sorted_idx]
                ys3_raw = [ys3_raw[i] for i in nonzero_sorted_idx]
                texts = [texts[i] for i in nonzero_sorted_idx]
                tokens = [tokens[i] for i in nonzero_sorted_idx]
                select_token_id = [select_token_id[i] for i in nonzero_sorted_idx]
            elif len(y_feats_dict) > 1:  # multi-speaker asr mode
                ys = zip(*[[y[i] for i in nonzero_sorted_idx] for y in ys])

            y_name = list(y_feats_dict.keys())[0]
            y_name2 = list(y2_feats_dict.keys())[0]
            y_name2_raw = list(y2_feats_raw_dict.keys())[0]
            y_name3 = list(y3_feats_dict.keys())[0]
            y_name3_raw = list(y3_feats_raw_dict.keys())[0]
            text_name = list(text_dict.keys())[0]
            token_name = list(token_dict.keys())[0]
            select_token_id_name = list(select_token_id_dict.keys())[0]

            # Keeping x_name and y_name, e.g. input1, for future extension
            return_batch = OrderedDict(
                [*[(x_name, x) for x_name, x in zip(x_names, xs)], (y_name, ys), (y_name2, ys2), (y_name2_raw, ys2_raw),
                 (y_name3, ys3), (y_name3_raw, ys3_raw), (text_name, texts), (token_name, tokens), (select_token_id_name, select_token_id)]
            )
        else:
...
class LoadInputsAndTargetsBERT(object):
    """Create a mini-batch from a list of dicts
    ...
    def __call__(self, batch):
        ...
        for uttid, info in batch:
            ...
            if self.load_output:
                ...
                for idx, inp in enumerate(info["output"]):
                    #print(idx, inp)
                    if "rec_tokenid" in inp:
                        # ======= Legacy format for output =======
                        # {"output": [{"tokenid": "1 2 3 4"}])
                        x = np.fromiter(
                            map(int, inp["rec_tokenid"].split()[:-1]), dtype=np.int64
                        )
                        #"rec_text": "▁WHICH▁AIRLINE▁IS▁U▁S<eos>"
                        txt = [word.lower() for word in inp["rec_text"].split('<eos>')[0].split('▁')[1:]]
                        #print(txt)
                        # "rec_token": "▁WHICH ▁AIRLINE ▁IS ▁U ▁S <eos>"
                        tokens = [token for token in inp["rec_token"].split(' <eos>')[0].split()]
                        #print('txt is', txt)
                        #print('tokens is', tokens)
                        if not txt:
                            print('empty txt and tokens!')
                            txt = ['_shit']
                            tokens = ['_shit']
                            print('modify empty txt and tokens to shit')
                        #print(tokens)
                        #"rec_text": "▁TO▁WHAT▁CITIES▁FROM▁BOSTON▁DOES▁AMERICA▁WEST▁FLY▁FIRST▁CLASS<eos>"
                        #"rec_token": "▁TO ▁WHAT ▁CITIES ▁FROM ▁BOSTON ▁DOES ▁AMERIC A ▁WEST ▁FLY ▁FIRST ▁CLASS <eos>",
                        select_token_id = [1 if token[0] == '▁' else 0 for token in tokens]
                        #print(select_token_id)
                    else:
                        # ======= New format =======
                        # {"input":
                        #  [{"feat": "some/path.h5:F01_050C0101_PED_REAL",
        ...
        # Doesn't return the names now.
        return list(return_batch.values())
    ...
    def _create_batch_asr(self, x_feats_dict, y_feats_dict, y2_feats_dict, y2_feats_raw_dict, y3_feats_dict, y3_feats_raw_dict,
        ...
        if self.load_output:
            if len(y_feats_dict) == 1 and len(y2_feats_dict) == 1:
                ...
                nonzero_idx = list(filter(lambda i: len(ys[i]) > 0, range(len(ys))))
                nonzero_idx2 = list(filter(lambda i: len(ys2[i]) > 0, range(len(ys2))))
                print(nonzero_idx, nonzero_idx2)
                #assert nonzero_idx == nonzero_idx2
