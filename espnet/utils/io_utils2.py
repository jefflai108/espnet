...
class LoadInputsAndTargets(object):
    ...
    def __call__(self, batch):
            ...
            if self.load_output:
                ...
                for idx, inp in enumerate(info["output2"]):
                    ...
                    if "slotsid" in inp:
                        ...
                        x = np.fromiter(
                            map(int, inp["slotsid"].split()), dtype=np.int64
                        )
        ...
        if self.mode == "asr":
            ...
            return_batch, uttid_list = self._create_batch_asr(
                x_feats_dict, y_feats_dict, y2_feats_dict, y3_feats_dict, uttid_list
            )
            #print(return_batch)
            #print(uttid_list)
        elif self.mode == "tts":
    ...
    def _create_batch_asr(self, x_feats_dict, y_feats_dict, y2_feats_dict, y3_feats_dict, uttid_list):
        ...
        if self.load_output:
            ...
            return_batch = OrderedDict(
                [*[(x_name, x) for x_name, x in zip(x_names, xs)], (y_name, ys), (y_name2, ys2), (y_name3, ys3)]
            )
        else:
