...
class Reporter(chainer.Chain):
    ...
    def report(self, loss_ctc, loss_ctc_2, loss_att, loss_att_2, acc, acc2,
               cer_ctc, cer_ctc2, cer, cer2, wer, wer2, mtl_loss, mtl_loss2):
        """Report at every step."""
        reporter.report({"loss_ctc": loss_ctc}, self)
        reporter.report({"loss_ctc2": loss_ctc_2}, self)
        reporter.report({"loss_att": loss_att}, self)
        reporter.report({"loss_att2": loss_att_2}, self)
        reporter.report({"acc": acc}, self)
        reporter.report({"acc2": acc2}, self)
        reporter.report({"cer_ctc": cer_ctc}, self)
        reporter.report({"cer_ctc2": cer_ctc2}, self)
        reporter.report({"cer": cer}, self)
        reporter.report({"cer2": cer2}, self)
        reporter.report({"wer": wer}, self)
        reporter.report({"wer2": wer2}, self)
        logging.info("mtl loss:" + str(mtl_loss))
        logging.info("mtl loss2:" + str(mtl_loss2))
        reporter.report({"loss": mtl_loss}, self)
        reporter.report({"loss2": mtl_loss2}, self)
