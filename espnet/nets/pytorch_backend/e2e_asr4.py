...
class Reporter(chainer.Chain):
    ...
    def report(self, loss_ctc, loss_att, loss_slots, acc, tag_acc, class_acc, cer_ctc, cer, wer, total_loss, total_acc):
        """Report at every step."""
        reporter.report({"loss_ctc": loss_ctc}, self)
        reporter.report({"loss_att": loss_att}, self)
        reporter.report({"loss_slots": loss_slots}, self)
        reporter.report({"acc": acc}, self)
        reporter.report({"tag_acc": tag_acc}, self)
        reporter.report({"class_acc": class_acc}, self)
        reporter.report({"cer_ctc": cer_ctc}, self)
        reporter.report({"cer": cer}, self)
        reporter.report({"wer": wer}, self)
        logging.info("toal loss:" + str(total_loss))
        reporter.report({"loss": total_loss}, self)
        reporter.report({"total_acc": total_acc}, self)
