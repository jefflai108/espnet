...
from espnet.asr.asr_utils_2 import add_results_to_json
from espnet.asr.asr_utils_2 import add_results_to_json2
...
from espnet.asr.pytorch_backend.asr_init2 import freeze_modules
from espnet.asr.pytorch_backend.asr_init2 import load_trained_model
from espnet.asr.pytorch_backend.asr_init2 import load_trained_modules
import espnet.asr.pytorch_backend.asr_init as asr_init
...
class CustomConverter(object):
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        assert len(batch) == 1
        xs, ys, ys2, ys3 = batch[0]

        # perform subsampling
        if self.subsampling_factor > 1:
            xs = [x[:: self.subsampling_factor, :] for x in xs]
        #print('subsampling factor is %f' % self.subsampling_factor)
        ...
        ys_pad3 = torch.from_numpy(np.array(ys3)).to(device)
        ...
        #print(ys_pad3)
        #print(ilens)
        return xs_pad, ilens, ys_pad, ys_pad2, ys_pad3
...
def train(args):
    ...
    odim2 = int(valid_json[utts[0]]["output2"][0]["shape"][-1])
    for i in range(args.num_encs):
        logging.info("stream{}: input dims : {}".format(i + 1, idim_list[i]))
    logging.info("#output dims: " + str(odim))
    logging.info("#output dims 2: " + str(odim2))
    print('odim is %s' % str(odim))
    print('odim2 is %s' % str(odim2)) # 141
    ...
    if args.mtlalpha == 1.0:
        ...
        print("Pure attention mode")
    else:
        print('metaalpha is %f\n' % args.mtlalpha)
        print('metaalpha2 is %f\n' % args.mtlalpha2)
        mtl_mode = "mtl"
        logging.info("Multitask learning mode")
        print("Multitask learning mode\n")

    if (args.enc_init is not None or args.dec_init is not None) and args.num_encs == 1:
        model = load_trained_modules(idim_list[0], odim, odim2, args, args.pretrained_decoder2)
        print('pretrained encoder and decoder\n')
    else:
        model_class = dynamic_import(args.model_module)
        model = model_class(
            idim_list[0] if args.num_encs == 1 else idim_list, odim, odim2, args
        )
    ...
    print(" Total parameter of the model = " + str(sum(p.numel() for p in model.parameters())) + '\n')
    ...
    with open(model_conf, "wb") as f:
        ...
        f.write(
            json.dumps(
                (idim_list[0] if args.num_encs == 1 else idim_list, odim, odim2, vars(args)),
                indent=4,
                ensure_ascii=False,
                sort_keys=True,
            ).encode("utf_8")
    ...
    #    print(k)
    #    print(v['output'][0]['text'], len(v['output'][0]['text'].split()))
    #    print(v['output'][0]['tokenid'], len(v['output'][0]['tokenid'].split()))
    ...
    if args.num_encs > 1:
        report_keys_loss_ctc = [
            "main/combined_loss_ctc{}".format(i + 1) for i in range(model.num_encs)
        ] + ["validation/main/combined_loss_ctc{}".format(i + 1) for i in range(model.num_encs)]
        report_keys_cer_ctc = [
    ...
    trainer.extend(
        extensions.PlotReport(
            [
                "main/joint_loss",
                "validation/main/joint_loss",
                "main/loss_ctc_data",
                "validation/main/loss_ctc_data",
                "main/loss_ctc_data2",
                "validation/main/loss_ctc_data2",
                "main/loss_att_data",
                "validation/main/loss_att_data",
                "main/loss_att_data2",
                "validation/main/loss_att_data2",
            ]
    ...
    trainer.extend(
        extensions.PlotReport(
            ["main/acc2", "validation/main/acc2"], "epoch", file_name="acc2.png"
        )
    )
    trainer.extend(
        extensions.PlotReport(
    ...
    trainer.extend(
        ...
        trigger=training.triggers.MinValueTrigger("validation/main/joint_loss"),
    )
        ...
        with torch.no_grad():
            for idx, name in enumerate(js.keys(), 1):
                ...
                if args.streaming_mode == "window" and args.num_encs == 1:
                    ...
                    #logging.info('char list 2 is')
                    #logging.info(train_args.char_list2)
                    #logging.info('char list 1 is')
                    #logging.info(train_args.char_list)

                    logging.info('decode ASR branch')
                    args2 = copy.deepcopy(args)
                    nbest_hyps, asr_hyp_length = model.recognize(
                        feat, args, train_args.char_list, rnnlm
                    )
                    logging.info(args.ctc_weight2)
                    logging.info('decode NLU branch')
                    nbest_hyps2 = model.recognize2(
                        feat, asr_hyp_length, args2, train_args.char_list2[:]
                    )

                new_js[name] = add_results_to_json(
                    js[name], nbest_hyps, train_args.char_list
                )
                new_js[name] = add_results_to_json2(
                    js[name], new_js[name], nbest_hyps2, train_args.char_list2[:]
                )
