...
import espnet.asr.asr_utils_2 as asr_utils_2
import espnet.asr.asr_utils as asr_utils
from espnet.asr.asr_utils_2 import torch_load
...
def load_trained_model(model_path):
    ...
    """
    idim, odim, odim2, train_args = asr_utils_2.get_model_conf(
    ...
    print('main module is' + model_module)
    model = model_class(idim, odim, odim2, train_args)
...
def get_trained_model_state_dict(model_path):
    ...
    idim, odim, args = asr_utils.get_model_conf(model_path, conf_path)
...
def load_trained_modules(idim, odim, odim2, args, pretrained_decoder2=False, interface=ASRInterface):
    ...
    def print_new_keys2(state_dict, modules, model_path):
        logging.warning("loading %s from model: %s", modules, model_path)

        new_state_dict = {}
        for k, v in state_dict.items():
            logging.warning("override %s" % k.replace('decoder.', 'decoder2.'))
            new_state_dict[k.replace('decoder.', 'decoder2.')] = v
    ...
    print('model_class is %s' % model_class)
    main_model = model_class(idim, odim, odim2, args)
            ...
            if os.path.isfile(model_path):
                ...
                if is_lm:
                    ...
                    if partial_state_dict:
                        print('partial state dict!')
                        if transfer_verification(
                            main_state_dict, partial_state_dict, modules
                        ):
                            # update decoder 1
                            print('updating decoder 1')
                            print_new_keys(partial_state_dict, modules, model_path)
                            main_state_dict.update(partial_state_dict)
                            # updater decoder 2
                            if pretrained_decoder2:
                                print('updating decoder 2')
                                new_partial_state_dict = print_new_keys2(partial_state_dict, modules, model_path)
                                #print(new_partial_state_dict)
                                main_state_dict.update(new_partial_state_dict)
                        else:
                            print('no transfer')
                            logging.warning(
