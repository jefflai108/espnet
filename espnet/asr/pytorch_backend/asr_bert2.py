...
from espnet.asr.asr_utils4 import add_results_to_json
...
from espnet.asr.pytorch_backend.asr_init4 import freeze_modules
from espnet.asr.pytorch_backend.asr_init4 import load_trained_model
from espnet.asr.pytorch_backend.asr_init4 import load_trained_modules
...
from espnet.utils.io_utils4 import LoadInputsAndTargets, LoadInputsAndTargetsBERT
...
# from slot tagger
from utils.bert_xlnet_inputs4 import prepare_inputs_for_bert_xlnet
from transformers import BertTokenizer, BertModel, XLNetTokenizer, XLNetModel
import models.slot_tagger_and_intent_detector_with_pure_transformer4 as joint_transformer
...
class CustomConverter(object):
    ...
    def __init__(self, tokenizer, subsampling_factor=1, dtype=torch.float32):
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        assert len(batch) == 1
        xs, ys, slots, slots_raw, classes, classes_raw, text, token, sp_select_token_id = batch[0]
        sp_select_token_id = [np.array(x) for x in sp_select_token_id]
        lens = [len(seq) for seq in slots]

        # deal with classes (> 1 class)
        classes = [x[0] if type(x) in {list, tuple} else x for x in classes]

        # double check
        # len(ys) == len(token_id) == len(tokens);
        # len(text) == len(slots) == len(ys[sp_select_token_id])
        for idx, y in enumerate(ys):
            #print('token')
            #print(y)
            #print('sp select token id')
            select_token_id = [bool(x) for x in sp_select_token_id[idx]]
            #print(y[select_token_id])
            assert len(text[idx]) == len(y[select_token_id])

        assert [len(y) for y in ys] == [len(x) for x in token], (ys, token)
        assert [len(x) for x in text] == [len(x) for x in slots], (text, slots)
        ...
        # get word_length of text
        word_length = [len(x) for x in text]

        #print(text)
        #print(word_length)
        inputs = prepare_inputs_for_bert_xlnet(text, word_length, self.tokenizer,
        ...
        # double check
        # len(sp_select_id) == len(bert_select_id) == len(text) == len(slots)
        bert_select_id = inputs["select_indexes"]
        for idx, i in enumerate(bert_select_id):
            assert len(i) == np.sum(sp_select_token_id[idx])
            assert len(i) == len(text[idx])
        ...
        slots_pad = pad_list(
            [
                torch.from_numpy(
                    np.array(slot[0][:]) if isinstance(slot, tuple) else slot
                ).long()
                for slot in slots
            ],
            0,
        ...
        sp_select_token_id_pad = pad_list(
            [
                torch.from_numpy(
                    np.array(select[0][:]) if isinstance(y, tuple) else select
                ).long()
                for select in sp_select_token_id
            ],
            0,
        ).to(device)

        #print(ys_pad)
        #print(sp_select_token_id)
        #print(ys_pad.shape, sp_select_token_id_pad.shape)
        #print(sp_select_token_id)
        #print(lens)

        return xs_pad, ilens, ys_pad, slots_pad, classes, inputs, lens, sp_select_token_id_pad
...
def train(args):
    ...
    if not args.slots_testing:
        ...
        tag_to_idx, idx_to_tag = vocab_reader.read_vocab_file(args.slots_read_vocab+'.tag', bos_eos=False, no_pad=True, no_unk=True)
        class_to_idx, idx_to_class = vocab_reader.read_vocab_file(args.slots_read_vocab+'.class', bos_eos=False, no_pad=True, no_unk=True)
    print(tag_to_idx)
    print(class_to_idx)
    print("Vocab size: %s %s" % (len(tag_to_idx), len(class_to_idx)))

    tokenizer = BertTokenizer.from_pretrained(args.slots_pretrained_model_name)
    pretrained_model = BertModel.from_pretrained(args.slots_pretrained_model_name)
    print(pretrained_model.config)
    model_tag_and_class = joint_transformer.Transformers_joint_slot_and_intent('bert', pretrained_model, len(tag_to_idx),
                        len(class_to_idx), dropout=args.slots_dropout, device=device, multi_class=args.slots_multiClass,
                        task_st=args.slots_task_st, task_sc=args.slots_task_sc, args=args)
    # read pretrained model
    if args.slots_read_model:
        model_tag_and_class.load_model(args.slots_read_model)
    else:
        assert not args.slots_testing
        #custom init (needed maybe) ...
        model_tag_and_class.init_weights(args.slots_init_weight)

    if (args.enc_init is not None or args.dec_init is not None) and args.num_encs == 1:
        model = load_trained_modules(idim_list[0], odim, args, model_tag_and_class, tag_to_idx, class_to_idx, device)
    else:
        model_class = dynamic_import(args.model_module)
        model = model_class(
            idim_list[0] if args.num_encs == 1 else idim_list, odim, args, model_tag_and_class, tag_to_idx, class_to_idx, device
        )
    ...
    print(" Total parameter of the model = "
          + str(sum(p.numel() for p in model.parameters())))
    model_tag_and_class_params = list(filter(lambda p: p.requires_grad, model_tag_and_class.parameters()))
    print(" Model tag and loss parameter = "
          + str(sum(p.numel() for p in model_tag_and_class_params)
    ))
    ...
    #model_tag_and_class_params = list(filter(lambda p: p.requires_grad, model_tag_and_class.parameters()))
    #total_model_params = list(model_params) + model_tag_and_class_params
    total_model_params = model_params

    # Setup an optimizer
    if args.opt == "adadelta":
        optimizer = torch.optim.Adadelta(
            total_model_params, rho=0.95, eps=args.eps, weight_decay=args.weight_decay
        )
    elif args.opt == "adam":
        optimizer = torch.optim.Adam(total_model_params, weight_decay=args.weight_decay)
    elif args.opt == "noam":
        from espnet.nets.pytorch_backend.transformer.optimizer import get_std_opt

        optimizer = get_std_opt(
            total_model_params, args.adim, args.transformer_warmup_steps, args.transformer_lr
        )
    else:
    ...
    # Setup a converter
    if args.num_encs == 1:
        converter = CustomConverter(tokenizer, subsampling_factor=model.subsample[0], dtype=dtype)
    else:
    ...
    trainer.extend(
        extensions.PlotReport(
            [
                "main/total_acc",
                "validation/main/total_acc",
                "main/acc",
                "validation/main/acc",
                "main/tag_acc",
                "validation/main/tag_acc",
                "main/class_acc",
                "validation/main/class_acc",
    ...
    trainer.extend(
        extensions.PlotReport(
            ["main/cer", "validation/main/cer",
             "main/wer", "validation/main/wer"]
            + ([] if args.num_encs == 1 else report_keys_loss_ctc),
            "epoch",
            file_name="wer.png",
        )
    ...
    if mtl_mode not in ["ctc", "transducer"]:
        trainer.extend(
            ...
            trigger=training.triggers.MaxValueTrigger("validation/main/total_acc"),
        )
...
def recog(args):
    ...
    model, train_args = load_trained_model(args.model, model_tag_and_class, tag_to_idx, class_to_idx, device)
    assert isinstance(model, ASRInterface)
    ...
    load_inputs_and_targets = LoadInputsAndTargetsBERT(
        tag_to_idx,
        class_to_idx,
        mode="asr",
        load_output=True,
    ...
    if args.batchsize == 0:
        with torch.no_grad():
            for idx, name in enumerate(js.keys(), 1):
                ...
                #### copy from custom_converter
                xs, ys, slots, slots_raw, classes, classes_raw, text, token, sp_select_token_id = feats
                if feats == [[], [], [], [], [], [], [], [], []]: # ASR recog is empty in this case
                    print('edge case: ASR recog is empty')
                    print('produce empty slots')
                    output_strs[name] = name + ' ' + ' ' + '<=>' + ' ' + '<=> ' + 'shit'
                    print('output_str', name + ' ' + ' ' + '<=>' + ' ' + '<=> ' + 'shit')
                    continue
                sp_select_token_id = [np.array(x) for x in sp_select_token_id]
                lens = [len(seq) for seq in slots]

                # deal with classes (> 1 class) ## modify this later
                classes = [x[0] if type(x) in {list, tuple} else x for x in classes]
                ...
                assert [len(y) for y in ys] == [len(x) for x in token], (ys, token)
                #assert [len(x) for x in text] == [len(x) for x in slots], (text, slots)
                print('slots length', [len(x) for x in slots])
                print('text length', [len(x) for x in text])

                ilens = np.array([x.shape[0] for x in xs])

                word_length = [len(x) for x in text]

                inputs = prepare_inputs_for_bert_xlnet(text, word_length, tokenizer,
                            cls_token_at_end=False,  # xlnet has a cls token at the end
                ...
                #print(xs_pad, ilens, ys_pad, slots_pad, classes, inputs, lens, sp_select_token_id_pad)
                ...
                if args.streaming_mode == "window" and args.num_encs == 1:
                    ...
                    # print(xs_pad, ilens, ys_pad, slots_pad, classes, inputs, lens, sp_select_token_id_pad)
                    output_str = model.forward_bert(
                        xs_pad, ilens, ys_pad, slots_pad, classes, inputs, word_length, sp_select_token_id_pad, idx_to_tag, idx_to_class, text, word_length)
                    output_str = name + ' ' + output_str
                    print('output_str', output_str)
                output_strs[name] = output_str
    ...
    with open(args.output_path, 'w') as f, torch.no_grad():
        for k, v in output_strs.items():
            f.write(v + '\n')
