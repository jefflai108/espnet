...
def get_model_conf(model_path, conf_path=None):
    ...
    if isinstance(confs, dict):
        ...
        # for asr, tts, mt
        idim, odim, odim2, args = confs
        return idim, odim, odim2, argparse.Namespace(**args)
...
def parse_nbest_hyps(nbest_hyps, char_list):
    ...
    for n, hyp in enumerate(nbest_hyps, 1):
        ...
        return rec_text, rec_token, rec_tokenid

def add_results_to_json2(js, new_js, nbest_hyps, char_list):
    """Add N-best results to json.
    ...
    for n, hyp in enumerate(nbest_hyps, 1):
        ...
        # copy ground-truth
        if len(js["output2"]) > 0:
            out_dic = dict(js["output2"][0].items())
        else:
        ...
        #out_dic["rec_text2"] = rec_text
        out_dic["rec_slots2"] = rec_token

        out_dic["rec_slotsid2"] = rec_tokenid
        out_dic["score2"] = score

        # add to list of N-best result dicts
        new_js["output2"].append(out_dic)

        # show 1-best result
        if n == 1:
            if "slots" in out_dic.keys():
                logging.info("groundtruth slots: %s" % out_dic["slots"])
            logging.info("prediction slots: %s" % out_dic["rec_slots2"])
    ...
    for n, hyp in enumerate(nbest_hyps, 1):
        ...
        if n == 1:
            if "text" in out_dic.keys():
                logging.info("groundtruth text: %s" % out_dic["text"])
            logging.info("prediction text: %s" % out_dic["rec_text"])
            logging.info("prediction token: %s" % out_dic["rec_token"])
